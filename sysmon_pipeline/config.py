...
class IOConfig:
    ...
    # Conditional-enrichment columns (gate LSASS/Run-key/Kerberoasting severity
    # and MITRE mappings in mapping.py — see build_tokens docstring)
    target_image_cols: Tuple[str, ...] = (
        "TargetImage", "target_image", "winlog.event_data.TargetImage")
    target_object_cols: Tuple[str, ...] = (
        "TargetObject", "target_object", "winlog.event_data.TargetObject")
    ticket_encryption_cols: Tuple[str, ...] = (
        "TicketEncryptionType", "ticket_encryption_type",
        "winlog.event_data.TicketEncryptionType")
    logon_type_cols: Tuple[str, ...] = (
        "LogonType", "logon_type", "winlog.event_data.LogonType")
...
class RoleConfig:
    ...
    # Hostname regex patterns for role assignment.
    # Each entry is (regex_pattern, role_name). First match wins.
    # Set to None to use the built-in DEFAULT_ROLE_PATTERNS.
    # Override with environment-specific patterns for your fleet.
    role_patterns: Optional[List[Tuple[str, str]]] = None

    # Feature columns used for rate feature computation (still used by
    # the frequency channel even though role inference no longer clusters)
...
class ScoringConfig:
    ...
    # Fusion method: 'borda' (rank fusion + corroboration gate) is the
    # framework's stated architecture and the default. 'weighted_linear'
    # remains available for ablation comparison.
    # NOTE: an earlier comment here claimed paper Eq. 15 is weighted_linear —
    # verify the paper text matches whichever method generates reported results.
    fusion_method: str = "borda"
    ...
    # Frequency-channel feature exclusions: cmdline-content flag rates are
    # owned by the context channel (component 2). Including them in the
    # IsolationForest matrix would make S_freq and S_ctx share input signal,
    # undermining the independence assumption behind the corroboration gate.
    freq_exclude_cols: Tuple[str, ...] = (
    ...
    # TF-IDF max features for cmdline novelty scoring
    tfidf_max_features: int = 5000

    # Max unique baseline commands to sample for TF-IDF similarity.
    # Higher = more accurate but slower. 1000 is a good tradeoff.
    tfidf_baseline_samples: int = 1000

    # Event severity grading (from v12_modular mapping.py)
    # Per-host temporal conformal calibration: number of time folds to split
    # the baseline window into for each host's benign S_seq distribution.
    # p-value resolution is 1/(calib_folds+1); needs a baseline long enough to
    # subdivide meaningfully (e.g. 6-8 folds over a 2-day baseline).
    calib_folds: int = 6
...
class StrataConfig:
    ...
    Usage:
        ...
        cfg = StrataConfig.fast()                     # reduced-cost dev mode
    """
    ...
    def apply_fast_preset(self) -> "StrataConfig":
        """
        Apply --fast settings for iterative development runs.
        Reduces bootstrap samples, IsolationForest trees, and TF-IDF features
        to cut runtime by 3-10× while keeping all channels and SHAP active.

        Does NOT disable any channels or analytical components — just reduces
        the computational cost of each. Paper-quality runs should use defaults.
        """
        self.baseline.bootstrap_samples = 200       # down from 1000
        self.scoring.iforest_n_estimators = 50      # down from 100
        self.scoring.tfidf_max_features = 300       # down from 5000
        self.scoring.tfidf_baseline_samples = 500   # down from 1000
        return self

    @classmethod
    def fast(cls) -> "StrataConfig":
        """Factory for a fast-preset config."""
        return cls().apply_fast_preset()
