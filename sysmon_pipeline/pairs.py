...
DEFAULT_INTERESTING_PAIRS: List[Tuple[int, int]] = [
    ...
    # NOTE: (4768, 4769) removed — TGT followed by a service ticket is the
    # normal Kerberos sequence on every domain logon, not a Kerberoasting
    # indicator. Kerberoasting detection requires the RC4 encryption type
    # on 4769, which is handled as a structural severity gate in mapping.py.
...
# A 4624->10 (logon -> LSASS access) on a domain controller is routine auth
# subsystem activity; the same pair on a workstation is more notable.
#
...
def correlate_critical_events_single_host(
    ...
    *,
    ...
    have their weights discounted.  A 4624→10 (logon → LSASS) pair on a DC
    contributes only ~20% of its normal weight; the same pair on a
    workstation gets the full 0.80 weight.
    ...
    for i in range(n):
        ev_i = int(evs[i])
        # Cap the inner scan to match compute_role_pair_baselines (i + 50).
        # Without this cap, scoring counted O(window_density^2) co-occurrences
        # per host — quadratic in event volume and asymmetric with the cap used
        # at baseline-learning time, so learned discounts didn't match observed
        # counts. The cap bounds it to the first 50 successors in-window.
        for j in range(i + 1, min(window_ends[i], i + 50)):
            key = (ev_i, int(evs[j]))
            if key in pair_set:
...
def _pair_tactic(src: int, dst: int) -> str:
    ...
    cred_access = {(10,1),(1,10),(10,3),(11,10),(7,10),(8,10),(4624,10)}
