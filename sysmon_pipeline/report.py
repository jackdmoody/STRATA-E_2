...
def _extract_report_data(art: "StrataArtifacts") -> dict:
    ...
    for _, row in triage.iterrows():
        triage_records.append({
            ...
            "S_seq_x_null":    round(_safe_float(row.get("S_seq_x_null", 0)), 2),
...
function render(){{
  ...
  var chans=[['Sequence',t.S_seq,(t.S_seq_x_null||0).toFixed(1)+'\u00d7 null'],['Frequency',t.S_freq,'IsolationForest'],['Context',t.S_ctx,'novelty = '+(t.cmdline_novelty||0).toFixed(2)],['Drift',t.S_drift,'vs. prior window']];
  ...
  h+='<div style="background:var(--s2);border:1px solid var(--bd);border-radius:8px;padding:12px 16px;font-size:12px;font-family:var(--mn)">p = '+(t.S_seq_pvalue||0).toFixed(4)+' (conformal) &middot; '+(t.S_seq_percentile||0).toFixed(0)+'th pct vs sampling null &middot; '+(t.S_seq_x_null||0).toFixed(1)+'\u00d7 null mean</div>';
  h+='</div>';
